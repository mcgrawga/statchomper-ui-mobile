...
export default function AddGameScreen({ navigation }) {
  ...
  // Shooting stats
  const [twoPointMade, setTwoPointMade] = useState(0);
  const [twoPointAttempts, setTwoPointAttempts] = useState(0);
  const [threePointMade, setThreePointMade] = useState(0);
  const [threePointAttempts, setThreePointAttempts] = useState(0);
  const [freeThrowMade, setFreeThrowMade] = useState(0);
  const [freeThrowAttempts, setFreeThrowAttempts] = useState(0);
  ...
  const showSuccessToast = (playerName, gameId) => {
    setShowToast(true);
  ...
  const handleSubmit = () => {
    ...
    try {
      ...
      showSuccessToast(playerName.trim(), gameId);
    } catch (error) {
    ...
    <SafeAreaView style={dynamicStyles.container}>
      ...
      {showToast && (
        ...
        >
          ...
          <Text style={dynamicStyles.toastText}>Game added successfully!</Text>
        </Animated.View>
      )}
      ...
      <View style={dynamicStyles.header}>
        ...
        <Text style={dynamicStyles.headerTitle}>Add New Game</Text>
        ...
        <View style={dynamicStyles.modalOverlay}>
          <View style={dynamicStyles.modalContent}>
            <View style={dynamicStyles.modalHeader}>
              <Text style={dynamicStyles.modalTitle}>Select or Add a Player</Text>
              <TouchableOpacity
